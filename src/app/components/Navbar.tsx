    ...
    <nav className="fixed top-0 w-full z-50 bg-[#0d0e12]/80 backdrop-blur-xl border-b border-white/5">
      <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
        ...
        <div className="flex items-center gap-2 group cursor-pointer">
          <span className="text-2xl font-bold text-purple-500 font-['Space_Grotesk'] group-hover:scale-105 transition-transform">{"</>"} FARHAN.DEV</span>
        </div>
        ...
        <div className="hidden md:flex items-center gap-8">
          {['Home', 'About', 'Skills', 'Contact'].map((item) => (
            <a 
              ...
              className={`text-sm font-['Space_Grotesk'] font-medium uppercase tracking-widest transition-colors hover:text-purple-400 ${item === 'Home' ? 'text-purple-400 border-b-2 border-purple-500 pb-1' : 'text-slate-400'}`}
            >
              {item}
            </a>
          ))}
